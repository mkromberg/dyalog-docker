@ECHO OFF
SETLOCAL
REM Start MiServer Container
REM Usage miserver-c directory [httpport] [rideport]
...
IF NOT EXIST %folder% (GOTO NOTEXIST) ELSE (
  IF %3=="" (
     START docker run -p %httpport%:8080 -it -v %folder%:/app dyalog/miserver:dbg
     ) ELSE (
     START docker run -p %httpport%:8080 -p %3:4502 -it -v %folder%:/app dyalog/miserver:dbg
     )
